...
- On an interactive terminal the report stays up and refreshes every 5 minutes until you press `q` (or Ctrl+C), with a `Press q to quit · r refresh` footer hint. Press `r` to reload immediately and restart the interval. `--refresh` sets the interval (30s-24h) and `--once` renders a single frame. A non-TTY stdout or stdin (pipes, CI, screenshots) always renders one frame and exits.
...
A normal invocation reports every Codex ChatGPT subscription it can discover from sibling entries in one Pi `auth.json`, and every Claude Code profile it can discover beside the process-selected one.
...
Claude Code stores one login per config directory (`CLAUDE_CONFIG_DIR`, default `~/.claude`).
quota-axi enrolls home-directory siblings named `.claude` or `.claude-*` / `.claude_*` when they look like a profile (they contain `.credentials.json` or `settings.json`), so `~/.claude` and `~/.claude-whc` are separate lanes.
`$QUOTA_AXI_CLAUDE_CONFIG_DIRS` adds directories that are not those home siblings (path-separated: `:` on POSIX, `;` on Windows).
The process-selected profile stays first.
Extra lanes never inherit `CLAUDE_CODE_OAUTH_TOKEN` or `CLAUDE_SECURESTORAGE_CONFIG_DIR`, which name the current session rather than a bystander directory.
A single profile keeps the legacy keyless report.
`--profile-only` still reads one native Claude file and never opens siblings.

```sh
quota-axi --provider claude --tui
quota-axi --provider claude --json
QUOTA_AXI_CLAUDE_CONFIG_DIRS=/path/to/other-profile quota-axi --provider claude --tui
```
...
A Claude lane's key is the config directory's basename without a leading dot (`.claude` → `claude`, `.claude-whc` → `claude-whc`).
...
Codex Pi sibling entries and Claude Code config-directory profiles are the discovery implementations on this tree.
